...
API_PATH = '/api/v4/open_api/cache_host'  # API路径
...
def host_flush(domain, urls):
    ...
    try:
        ...
        if response.status_code != 200 or data['status'] == 'error':
            print('缓存整站错误，当前状态码为: %s, 返回值为: %s' % (
                response.status_code, data))
        else:
            print('缓存整站成功，当前状态码为: %s, 返回值为: %s' % (
                response.status_code, data))
        return data
...
if __name__ == '__main__':
    host_flush('example.com', ['http://www.example.com'])
